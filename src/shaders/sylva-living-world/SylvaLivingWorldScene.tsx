...
export function SylvaLivingWorldScene({
  ...
  // Mount once and keep mounted for the life of the page; throttled via postMessage when off-screen
  const mounted = true;
    ...
    >
      {mounted ? (
        <iframe
          ...
          onLoad={() => {
            setReady(true);
            iframeRef.current?.contentWindow?.postMessage({ inView: hostVisible }, "*");
          }}
          style={{
            position: "absolute",
            inset: 0,
