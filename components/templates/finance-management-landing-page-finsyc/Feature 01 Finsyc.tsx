...
function FeatureCard({ title, description, icon: Icon, uiSrc, width: _width, className = "", delay = 0, isMounted = false }: FeatureCardProps) {
  return (
    ...
    >
      <div className="relative w-full h-[320px] sm:h-[400px] lg:h-[440px] overflow-hidden flex items-center justify-center p-6 sm:p-8 bg-[#F8FAFB]">
        {isMounted && (
          <video
            autoPlay
            loop
      ...
      <div className="p-6 sm:10 flex flex-col sm:flex-row items-start gap-5 self-stretch bg-white">
        <div className="w-10 h-10 p-2 flex items-center justify-center border border-[#188E39]/20 bg-[#188E39]/5 rounded-lg shrink-0">
          <Icon className="w-6 h-6 text-[#188E39] stroke-[3px]" />
        </div>
...
export default function OriginaFinsycFeature({ className }: { className?: string }) {
  ...
  return (
    <>
      ...
      <section className={"w-full bg-[#FFFEFF] py-20 lg:py-32 overflow-hidden " + (className || "")}>
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="flex flex-col items-center">
              <motion.div
                ...
                className="flex items-center gap-2 px-4 py-1.5 rounded-full border border-[#188E39]/10 bg-[#188E39]/5 whitespace-nowrap"
              >
