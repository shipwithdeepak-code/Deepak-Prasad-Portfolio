...
const getTagStyles = (tag: string): string => {
  ...
  if (t === "ai") return "bg-[#E0F3FE] text-[#0268A1]";
  if (t === "innovation") return "bg-[#DCFCE7] text-[#15803D]";
  if (t === "tech") return "bg-[#FEF2C6] text-[#B45209]";
...
function BlogCard({ image, date, title, description, tags, imageTop = true, delay = 0 }: BlogCardProps) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group cursor-pointer flex flex-col items-start bg-white hover:bg-[#F7FDFE] rounded-[24px] overflow-hidden border border-[#052618]/10 shadow-[0_4px_24px_rgba(4,39,24,0.02)] hover:shadow-[0_20px_60px_rgba(4,39,24,0.08)] transition-all duration-500 w-full lg:w-[612px]"
    >
      {imageTop && (
      ...
      <div className="flex flex-col p-6 md:p-[40px] gap-4 w-full self-stretch lg:w-[612px]">
        <span className="text-[#052618] font-sans text-base md:text-[18px] leading-[28px] opacity-80">
          {date}
        </span>
        <h3 className="text-[#052618] font-onest text-[28px] md:text-[34px] font-semibold leading-[1.1] md:leading-[38px] tracking-[-1px]">
          {title}
        </h3>
        <p className="text-[#052618] font-sans text-base md:text-[18px] leading-[28px] opacity-80">
          {description}
        </p>
...
export default function Blog01Finsyc({ className }: { className?: string }) {
  return (
    <>
      ...
      <section className={"w-full bg-[#FFFEFE] py-16 md:py-[100px] flex justify-center " + (className || "")}>
          ...
          <div className="w-full max-w-[1248px] mx-auto">
            ...
            <div className="flex flex-col items-center text-center mb-12 md:mb-[80px]">
              <motion.div
                ...
                viewport={{ once: true }}
                className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#E4F2EA] border border-[#138E5E]/10 mb-6"
              >
                <Sparkles className="w-3.5 h-3.5 text-[#138E5E]" />
                <span className="text-[#138E5E] text-[13px] font-sans font-medium uppercase tracking-wider">Latest posts</span>
              </motion.div>
              ...
              <motion.h2
                ...
                viewport={{ once: true }}
                className="text-[32px] sm:text-[40px] md:text-[52px] font-onest font-semibold text-[#052618] leading-[1.1] md:leading-[58px] tracking-tight md:tracking-[-2px] mb-6 max-w-3xl"
              >
              ...
              <motion.p
                ...
                viewport={{ once: true }}
                className="text-[15px] md:text-[18px] text-[#052618] leading-[1.6] md:leading-[28px] max-w-[612px] font-sans opacity-60"
              >
